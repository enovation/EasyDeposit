 ...
 * Rev:  1.2.10
 */
	$.fn.mcDropdown = function(list, options, indexid) {
		// track the dropdown object
		var dd;

		// create a dropdown for each match
		this.each(function() {
			dd = $.data(this, 'mcdropdown_'+indexid);
			
			// we're already a dropdown, return a reference to myself
			if( dd ) return false;

			new $.mcDropDownMenu(this, list, options, indexid);
		});
	...
	$.mcDropDownMenu = function(el, list, options, indexid){
			...
			.bind("click", function(e){
				...
				if( $ul.length ){
					...
					if( settings.click != null ){
						...
						if (settings.click.apply($target, [e, thismenu, settings]) == false) {
                                                    return false;
                                                }
					}
				}
		...
		$divInput = $('<div class="mcdropdown_'+indexid+' mcdropdown" name="mcdropdown"><a href="#" tabindex="-1">UU</a><input type="hidden" name="' + (el.name || el.id) + '" id="' + (el.id || el.name)+ '" value="'+el.value+'"/></div>')
			.appendTo($('<div style="position: relative; float:left; "></div>'))
			.parent();
		...
		$divInput = $divInput.find('.mcdropdown_'+indexid).prepend($input);
		...
		$.data($hidden[0], 'mcdropdown_'+indexid, thismenu);
		...
		this.setValue = function (value, skipCallback){
			// update the hidden value
                        ...
                        if ($hidden.attr('id') != 'keyword'+$('#current_menu').val()) {
                            return false;
                        }
		...
		function onFocus(e){
                        var indexid = $self.attr('id').split('_');
                        if (indexid[1] != null) {
                           $('#current_menu').val(indexid[1]);
                        }
